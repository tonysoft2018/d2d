<script>
    ...
    /*<Events  Button ButtonUpdate # 2 >*/
        const ButtonUpdate  = ( 
            ...
            let ArraysRoles = JSON.parse(localStorage.getItem('JSON_USUARIO'));
            ...
            if(ArraysRoles.length > 0){
                ...
                for(let i = 0; i< ArraysRoles.length; i++){
                    if(ArraysRoles[i].tipoPerfil == "SOCIO CLIENTE"){
                        if(ArraysRoles[i].idUsuario == idUsuario){
                            option  += `<option value="${ArraysRoles[i].idUsuario}" selected > ${ArraysRoles[i].nombres}  ${ArraysRoles[i].apellidos} </option>`;
                        }else{
                            option  += `<option value="${ArraysRoles[i].idUsuario}" > ${ArraysRoles[i].nombres}  ${ArraysRoles[i].apellidos} </option>`;
                        }
                    }
                }          
                $('#update-usuario-door2door').html(option);                          
            } 
